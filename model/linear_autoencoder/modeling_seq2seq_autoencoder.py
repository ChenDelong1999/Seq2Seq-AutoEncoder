...
class Seq2SeqAutoEncoderModel(Seq2seqAutoencoderPreTrainedModel):
    def __init__(self, config: Seq2SeqAutoEncoderConfig):
        ...
        self.input_dim = config.input_channels * config.model_seq_length
        ...
        self.decoder = torch.nn.Sequential(
            nn.Linear(config.d_model, self.input_dim),
            nn.Sigmoid()
        )
    ...
    def enable_caching_latents(self, cache_dir, use_existing=False, cache_limit=10000000):
        print('linear autoencoder do not need cache')
    ...
    def encode(self, inputs_embeds):
        # inputs_embeds: [batch_size, seq_len, 5]

        inputs_embeds = inputs_embeds.reshape(inputs_embeds.shape[0], -1)
        latents = self.encoder(inputs_embeds)
        
        return latents
    
    def decode(self, latents, inputs_embeds):
        
        return self.decoder(latents).reshape(inputs_embeds.shape)
